export default {
  ...
  /**
   * 生成随机字母
   * @param {String} type 要生成的类型 number string 默认字母和数字混合
   * @param {Number} length 长度，默认4
   * @return code
   */
  randomCode(type, length = 4) {
    let code = ''
    if (type === 'number') {
      ...
      return Math.random().toString(36).substr(2).substring(0, length)
    }
  }
}